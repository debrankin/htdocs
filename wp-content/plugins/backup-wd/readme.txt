...
[Backup WD](https://10web.io/services/backup/)     
...
Click on **Backup WD** menu item, then navigate to **Jobs** page. This page of Backup WD plugin contains the summary of all created backup jobs. You can run them anytime, edit, copy or delete them.
...
**Send Email.** Select this option to send email containing the log of this backup job. You can the recipient email address, and other email options from **Backup WD > Settings > Email Options** tab.
...
**Manually.** To run your backup job manually, save the created job first, then navigate to **Backup WD > Jobs** page. Hover on the title of backup job you need to run, then click Run Now.
...
Backup WD plugin is the perfect tool to keep database backups also. **DB Backup** tab of new job editor dashboard lets you configure the backup of necessary database tables.
...
Before configuring Google Drive as the destination of this backup job, make sure to add **Google API keys** to the plugin. This can be done from **Backup WD > API Keys > Google Drive** page.
...
To configure Dropbox as the destination for your backup files, you will need to add its **API keys** to **Backup WD > API Keys > Dropbox** page.
...
After downloading the database backup file, you are able to easily restore it by uploading the package through Backup WD plugin. Go to Backup WD > Restore page, simply press **Choose File** and browse the backup file you wish to restore, then click **Restore.**
