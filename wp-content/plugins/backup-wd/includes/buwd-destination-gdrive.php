    ...
    public function run(Buwd_Job $job_object)
    {
        ...
        try {
            ...
            if (empty($job_object->steps_data[$job_object->current_step]['session_uri'])) {
                ...
                $response = $http_client->request($url, array(
                    'method'  => 'POST',
                ...
                if ($response['response']['code'] == 200 || $response['response']['code'] == 201) {
                    if (isset($response['headers']['location'])) {
                        $job_object->steps_data[$job_object->current_step]['session_uri'] = trim($response['headers']['location']);
                    }
                }
            ...
            if ($handle = fopen($job_object->backup_folder . $job_object->backup_file, 'rb')) {
                ...
                while (!feof($handle)) {
                    ...
                    $response = $http_client->request($job_object->steps_data[$job_object->current_step]['session_uri'], array(
                        'method'  => 'PUT',
                        'body'    => $chunk_data,
                        'headers' => array(
                            'Authorization'  => 'Bearer ' . $access_token['access_token'],
                    ...
                    update_site_option('response_gdrive',$response);
                    if ($response['response']['code'] == 200 || $response['response']['code'] == 201 || $response['response']['code'] == 308) {
                        //$json_data = mb_substr($response['body'], $curl_header_size);
                        // $uploaded_file = json_decode($json_data);
                        $uploaded_file = json_decode($response['body']);
                        if ($response['response']['code'] == 308) {
