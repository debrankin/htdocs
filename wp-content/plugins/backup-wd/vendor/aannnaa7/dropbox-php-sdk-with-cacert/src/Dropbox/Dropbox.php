...
class Dropbox
{
    ...
    public function upload($dropboxFile, $path, array $params = [])
    {
        ...
        if(!isset($drop_job_object->steps_data[$drop_job_object->current_step]['dropbox_file'])){
            $dropboxFile = $this->makeDropboxFile($dropboxFile);
            $drop_job_object->steps_data[$drop_job_object->current_step]['dropbox_file'] = $dropboxFile;
        }
        
        //If the file is larger than the Chunked Upload Threshold
        if ($drop_job_object->steps_data[$drop_job_object->current_step]['dropbox_file']->getSize() > static::AUTO_CHUNKED_UPLOAD_THRESHOLD) {
            //Upload the file in sessions/chunks
            return $this->uploadChunked($drop_job_object->steps_data[$drop_job_object->current_step]['dropbox_file'], $path, null, null, $params);
        }

        //Simple file upload
        return $this->simpleUpload($drop_job_object->steps_data[$drop_job_object->current_step]['dropbox_file'], $path, $params);
    }
    ...
    public function uploadChunked($dropboxFile, $path, $fileSize = null, $chunkSize = null, array $params = array())
    {
       ...
       // $sessionId = $this->startUploadSession($dropboxFile->getFilePath(), $chunkSize);
        ...
        while ($drop_job_object->steps_data[$drop_job_object->current_step]['remaining'] > $chunkSize) {
            ...
            $drop_job_object->check_if_stopped();
